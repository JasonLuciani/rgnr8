...
class SqlAuditLog:
    ...
    def create_schema(self) -> None:
        # AUTOINCREMENT is sqlite-only syntax -- Postgres rejects it outright with
        # `syntax error at or near "AUTOINCREMENT"`, so this DDL could never run
        # there and audit_event was simply absent on a Postgres deployment. Use
        # the engine's own identity syntax. `record()` already reads the key back
        # with RETURNING seq, which both engines support, so nothing else changes.
        # (``placeholder`` is this codebase's dialect signal: "%s" psycopg, "?" sqlite.)
        seq = ("seq BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
               if self._ph == "%s" else
        ...
        try:
            cur.execute(
                ...
                f"({seq}, actor TEXT NOT NULL, action TEXT NOT NULL, "
